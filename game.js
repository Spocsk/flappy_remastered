...
class playGame extends Phaser.Scene {
    ...
    preload() {
        this.load.image('bird', './assets/PNG/fly.png');
        this.load.image('pipe', './assets/PNG/a.png');
        this.load.image('button', './assets/PNG/Letters/letterR.png')
    }

    create() {

        const clickButton = this.add.text(10, 450, 'Resume', {fill: '#0f0', backgroundColor: '#0f0000'})
            .setInteractive()
        ...
        this.pipePool = [];
        for (let i = 0; i < 4; i++) {
            this.pipePool.push(this.pipeGroup.create(0, 0, 'pipe').setScale(0.5));
            this.pipePool.push(this.pipeGroup.create(0, 0, 'pipe').setScale(0.8));
            this.pipePool.push(this.pipeGroup.create(0, 0, 'pipe'));
            this.placePipes(false);
        }
        this.pipeGroup.setVelocityX(-gameOptions.birdSpeed);
        this.bird = this.physics.add.sprite(80, game.config.height / 2, 'bird').setScale(0.08);
    ...
    resume() {
        ...
        this.bool = !this.bool
        if (!this.bool) {
            gameOptions.birdFlapPower = 0
            gameOptions.birdGravity = 0
            gameOptions.birdSpeed = 0
        } else {
            gameOptions.birdFlapPower = 800
            gameOptions.birdGravity = 125
            gameOptions.birdSpeed = 300
        }
